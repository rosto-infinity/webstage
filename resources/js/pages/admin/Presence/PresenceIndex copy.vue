...
import { ChevronLeft, ChevronRight, Download, Filter, Search, Users, Calendar, Clock, Pen, X } from 'lucide-vue-next';

// Typage des messages flash
interface FlashMessages {
  success?: string;
  error?: string;
  warning?: string; // Optionnel : pour anticiper d'autres types de messages
}


// Messages flash avec typage explicite
const flash = computed<FlashMessages>(() => usePage().props.flash as FlashMessages);
const showFlash = ref(false);
const flashMessage = ref('');
const flashType = ref<'success' | 'error' | 'warning'>('success'); // Type étendu

// Surveillance réactive avec gestion complète
watch(flash, (newVal: FlashMessages) => {
  ...
  if (hasMessage) {
    ...
    // Durée augmentée à 5s pour une meilleure expérience utilisateur
  ...
  immediate: true,
  deep: true // Surveillance profonde pour les changements imbriqués
});
// Données de test
...
const data = ref<AttendanceRecord[]>([
  { id: 1, date: '2025-06-17', fullName: 'Alice Dupont', email: 'alice.dupont@email.com', arrivalTime: '08:05', departureTime: '17:00', lateMinutes: 5, isAbsent: false, isLate: true },
  { id: 2, date: '2025-06-17', fullName: 'Benoît Martin', email: 'benoit.martin@email.com', arrivalTime: '08:00', departureTime: '17:00', lateMinutes: 0, isAbsent: false, isLate: false },
]);

// Statistiques
const presentCount = computed(() => data.value.filter(record => !record.isAbsent && !record.isLate).length);
const absentCount = computed(() => data.value.filter(record => record.isAbsent).length);
const lateCount = computed(() => data.value.filter(record => record.isLate).length);
...
const filteredAndSortedData = computed(() => {
  const filtered = data.value.filter(record => {
              ...
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th 
                    ...
                    class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                    <div class="flex items-center gap-1">Nom et Prénom</div>
                  </th>
                  <th 
                    @click="handleSort('email')"
                    class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                    <div class="flex items-center gap-1">E-mail</div>
                  </th>
                  <th 
                    @click="handleSort('arrivalTime')"
                    class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                    <div class="flex items-center gap-1">Arrivée</div>
                  </th>
                  <th 
                    @click="handleSort('departureTime')"
                    class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                    <div class="flex items-center gap-1">Départ</div>
                  </th>
                  <th 
                    @click="handleSort('lateMinutes')"
                    class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                    <div class="flex items-center gap-1">Retard (min)</div>
                  </th>
                  <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Absence</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr 
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <span :class="{ 'text-orange-600 font-medium': record.lateMinutes > 0, 'text-gray-900': record.lateMinutes <= 0 }">
                      {{ record.lateMinutes > 0 ? record.lateMinutes : 0 }} min
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span :class="['px-3 py-1 rounded-full text-xs font-medium', record.isAbsent ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800']">
                      {{ record.isAbsent ? 'Oui' : 'Non' }}
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span :class="['px-3 py-1 rounded-full text-xs font-medium', record.isLate ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800']">
                      {{ record.isLate ? 'Oui' : 'Non' }}
                    </span>
                  </td>
                </tr>
              </tbody>
